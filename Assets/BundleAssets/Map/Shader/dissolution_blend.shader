...
Shader "TY/FX/Dissolution_Blend" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float mask = (i.vertexColor.a*_N_mask);
                float4 _T_mask_var = tex2D(_T_mask,TRANSFORM_TEX(i.uv0, _T_mask));
                float thold = step(mask,_T_mask_var.r);
                float thold2 = 1 - thold;
                float node_6272 = 1.0;
                float node_5217_if_leB = step((_T_mask_var.r+_N_BY_KD),mask);
                float node_1274 = thold2-node_5217_if_leB;
                float3 finalColor = emissive + ((node_1274*_C_BYcolor.rgb)*_N_BY_QD);
                return fixed4(finalColor,(_TintColor.a*(_MainTex_var.a*(thold2+node_1274))));
            }
            ENDCG
        }
    }
