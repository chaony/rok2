...
//		}
		...
		string common = SprotoPath + "/Common.sproto";


		if (!File.Exists(common))
		{
			Debug.LogError("请更新协议GIT");
		}
	    
		string[] commonLines = File.ReadAllLines(common);
	    
	    
		string sproto = SprotoPath + "/Protocol.sproto";

		string[] sprotoLines = File.ReadAllLines(sproto);

		var targetFile = Application.dataPath + "/Protocol/Protocol.sproto";
		...
		newFileLines.AddRange(commonLines);
		newFileLines.AddRange(sprotoLines);
	    
		File.WriteAllLines(targetFile,newFileLines);
