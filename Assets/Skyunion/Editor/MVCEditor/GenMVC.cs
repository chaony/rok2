...
{
    /// <summary>
    /// 0->mediator,1->view,2->singleView,3->globalMediator,4->cmd,5->proxy
    /// </summary>
    public int type;
    ...
    public GenInfo(int type,string prefabPath,string codePath,string mediatorPath = "")
    {
        this.type = type;
...
public class GenMVC : ScriptableObject
{
    ...
    private static string m_s_tempRoot = "Assets/Skyunion/Editor/MVCEditor/Template/";
    private static string m_s_GenCodeDir = Application.dataPath + "/Scripts/Hotfix/";
    private static string m_s_CommonViewCodeDir = Application.dataPath + "/Scripts/Hotfix//MVC/View_Mediator/Common/View/";
    private static string m_s_CommonLogicCodeDir = Application.dataPath + "/Scripts/Hotfix//MVC/View_Mediator/Common/Logic/";
    ...
    public static void CreatePrCSByObj(GameObject obj,string outSavePath = "",bool isIL = false)
    {
        ...
        ViewBinder vb = prefab.GetComponent<ViewBinder>();
        if (vb == null)
        {
            vb = prefab.AddComponent<ViewBinder>();
        }
            ...
            if (bIsPrefab || (trans.name.IndexOf("_")>-1 && trans.parent))
            {
                ...
                if(bIsPrefab)
                {
                    ...
                    if (!nameDic.ContainsKey(member_name) && isTrue)
                    {
                        ...
                        string find = string.Format("\t\t\tm_{0} = new {1}_SubView(FindUI<RectTransform>(vb.transform ,\"{2}\"));", member_name, classType, path);
                    ...
                    for (int i = 0; i < coms.Length; i++)
                    {
                        ...
                        if (!nameDic.ContainsKey(member_name) && isTrue)
                        {
                            ...
                            string find = string.Format("\t\t\tm_{0} = FindUI<{1}>(vb.transform ,\"{2}\");", member_name, simpleName, path);
                            members.AppendLine(member);
                            finds.AppendLine(find);
                        }
                        else
                ...
                else
                {
                    ...
                    if (!nameDic.ContainsKey(member_name) && isTrue)
                    {
                        ...
                        string find = string.Format("\t\t\tm_{0} = FindUI<{1}>(vb.transform ,\"{2}\");", member_name, classType, path);
                        members.AppendLine(member);
                        finds.AppendLine(find);
                    }
                    else
        ...
        {
            GenInfo info = CreateTemplate(obj,savepath, m_s_tempRoot + "ViewTemplate.txt", members.ToString(), finds.ToString(),bindInfo);
            string mdPath = savepath.Replace("View.cs", "Mediator.cs");
    ...
    [MenuItem("Assets/Skyunion/MVC/Gen Prefab Code", false, 78)]
    ...
    [MenuItem("Assets/Skyunion/MVC/Gen SubView Code", false, 78)]
    ...
    [MenuItem("Assets/Skyunion/MVC/Gen Prefab Code Single View", false, 79)]
    ...
    [MenuItem("Skyunion/MVC/Gen GlobalMediator", false, 78)]
    public static void CreateGlobalMediatorCS()
    {
        string savepath = EditorUtility.SaveFilePanel(
            "Save GlobalMediator",
            m_s_GenCodeDir + "MVC/GlobalMediator/",
            "NewGlobalMediator",
            "cs");
        CreateTemplate(null,savepath,m_s_tempRoot + "GlobalMediatorTemplate.txt",string.Empty,string.Empty,string.Empty);
    }

    [MenuItem("Skyunion/MVC/Gen CMD", false, 80)]
    public static void CreateCMDCS()
    { 
        string savepath = EditorUtility.SaveFilePanel(
            "Save CMD",
            m_s_GenCodeDir + "MVC/CMD/",
            "NewCMD",
            "cs");        
        CreateTemplate(null,savepath,m_s_tempRoot + "CmdTemplate.txt",string.Empty,string.Empty,string.Empty);            
    }

    [MenuItem("Skyunion/MVC/Gen Proxy", false, 81)]
    public static void CreateProxyCS()
    {
        string savepath = EditorUtility.SaveFilePanel(
            "Save GlobalMediator",
            m_s_GenCodeDir + "MVC/Proxy/",
            "NewProxy",
            "cs");        
        CreateTemplate(null,savepath,m_s_tempRoot + "ProxyTemplate.txt",string.Empty,string.Empty,string.Empty);          
    }
    ...
    private static Dictionary<string, GenInfo>  getGenInfos(){
        ...
        if (s_genInfoDic == null)
        {
            ...
            {
                s_genInfoDic = JsonMapper.ToObject<Dictionary<string, GenInfo>>(File.ReadAllText(infoPath));
            }
        }
    ...
    private static void SaveGenInfo(){
        ...
        {           
            File.WriteAllText(infoPath,JsonTree(JsonMapper.ToJson(s_genInfoDic)));
        }
    }
    ...
    public static string JsonTree(string json)
    {
        ...
        List<char> jsonTree = new List<char>();
        for (int i = 0; i < json.Length; i++)
        {
            char c = jsonArr[i];
            if (level > 0 && '\n' == jsonTree[jsonTree.Count - 1])
            {
                jsonTree.AddRange(TreeLevel(level));
            }
            switch (c)
            {
                case '{':
        ...
        return new string(jsonTree.ToArray());
    }
    ...
    private static GenInfo CreateTemplate(GameObject obj,string createNewFilePath, string tempFilePath, string member, string memberfind, string binderInfo)
    {
        ...
        if(fileNameWithoutExtension.Substring(fileNameWithoutExtension.Length - 4, 4) == "View")
        //if (fileNameWithoutExtension.IndexOf("View") > -1)
        {
            string MediatorName = fileNameWithoutExtension.Remove(fileNameWithoutExtension.Length - 4, 4);
