...
namespace Sproto
{
	public class SprotoTypeDeserialize {
		...
		private UInt32 read_array_size() {
			...
			if (sz < 0)
                SprotoTypeSize.error (string.Format("error array size({0})",sz));
		...
		public Int64 read_integer() {
			if (this.value >= 0) {
				...
				if (sz == sizeof(UInt32)) {
                    ...
                    SprotoTypeSize.error (string.Format("read invalid integer size ({0})",sz));
				}
			}
		...
		public List<Int64> read_integer_list() {
			...
			if (len == sizeof(UInt32)) {
				if (sz % sizeof(UInt32) != 0) {
                    SprotoTypeSize.error (string.Format("error array size({0})@sizeof(Uint32)",sz));
				}
				...
				if (sz % sizeof(UInt64) != 0) {
                    SprotoTypeSize.error (string.Format("error array size({0})@sizeof(Uint64)",sz));
				}
                ...
                SprotoTypeSize.error (string.Format("error intlen({0})",len));
			}
		...
		public T read_obj<T>(string fullname = "") where T : SprotoTypeBase, new() {
			...
			//UnityEngine.Log.Info("read_obj fullname: "+fullname);
			var objHotfix = CoreUtils.hotService.Instantiate<T>(fullname);
			objHotfix.init(reader);
			return objHotfix;
		}

		private T read_element<T>(SprotoTypeReader reader, UInt32 sz, out UInt32 read_size,string fullname = "") where T : SprotoTypeBase, new() {
			read_size = 0;
			if (sz < SprotoTypeSize.sizeof_length) {
				SprotoTypeSize.error ("error array size.");
			}
			...
			if (CoreUtils.hotService.GetHotfixMode() != HotfixMode.ILRT)
			{
			 ...
			 //UnityEngine.Log.Info("read_element fullname: "+fullname);
			var objHotfix = CoreUtils.hotService.Instantiate<T>(fullname);
			objHotfix.init(reader);
		...
		public List<T> read_obj_list<T>(string fullname = "") where T : SprotoTypeBase, new(){
			UInt32 sz = this.read_array_size ();
			...
			for (UInt32 i = 0; sz > 0; i++) {
				UInt32 read_size;
				obj_list.Add (read_element<T> (reader, sz, out read_size,fullname));
				sz -= read_size;
			}
		...
		public Dictionary<TK, TV> read_map<TK, TV>(gen_key_func<TK, TV> func,string fullname = "") where TV : SprotoTypeBase, new() {
			UInt32 sz = this.read_array_size ();
			...
			for (UInt32 i = 0; sz > 0; i++) {
				UInt32 read_size;
				TV v = read_element<TV> (reader, sz, out read_size,fullname);
				TK k = func (v);
				map.Add (k, v);
				sz -= read_size;
			}
