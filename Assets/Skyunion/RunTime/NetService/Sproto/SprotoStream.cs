...
namespace Sproto
{
	public class SprotoStream
	{
		...
		private void _expand(int sz=0) {
			if(this.size - this.pos  < sz) {
				...
				if (this.size >= SprotoTypeSize.encode_max_size) {
                    SprotoTypeSize.error (string.Format("object is too large (>{0})", SprotoTypeSize.encode_max_size));
				}
		...
		public byte this[int i] {
			get {
				if (i < 0 || i >= this.size) {
                    throw new Exception (string.Format("invalid idx:{0} @get",i));
				}
			...
			set {
				if (i < 0 || i >= this.size) {
                    throw new Exception (string.Format("invalid idx:{0} @set", i));
				}
				this.buffer [i] = value;
			}
		}
