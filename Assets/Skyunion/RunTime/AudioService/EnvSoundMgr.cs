...
namespace Skyunion
{
    internal class EnvSoundMgr
    {
        ...
        public void Update()
        {
            ...
            UpdateSound(list, lodDistance);
        }

        private void UpdateSound(List<EnvSoundObj> list, float lodDistance)
        {
            for (int j = 0; j < list.Count; j++)
            {
                ...
                if (m_curPlayCount <= MaxPlayCount && envHandler.m_name != string.Empty && lodDistance <= envHandler.m_maxDxf)
                {
                    if (EnvSoundObj2.AudioHandler == null)
                    {
                        ...
                        {
                            CoreUtils.audioService.PlayOneShotAtPos(envHandler.m_name, envHandler.transform.position, (AudioHandler handler) =>
                            {
                                ...
                                else
                                {
                                    for (int k = list.Count - 1; k > j; k--)
                                    {
                                        ...
                                        if (EnvSoundObj3.AudioHandler != null && EnvSoundObj3.EnvHandler.m_name == envHandler.m_name)
                                        {
                                            ...
                                            {
                                                AudioHandler.gameObject.transform.position =
                                                    envHandler.transform.position;
                                            }
                ...
                {
                    if (EnvSoundObj2.AudioHandler.gameObject != null)
                    {
