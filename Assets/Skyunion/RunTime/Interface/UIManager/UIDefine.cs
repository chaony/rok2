...
namespace Skyunion
{
    ...
    {
        kNone = 0,              // 不创建蒙版
        kOnlyShow,              // 只显示,点击不做响应   （注：会创建灰色蒙板）
        kTouchClose,            // 点击蒙版关闭界面      （注：会创建灰色蒙板）
    ...
    public class UIInfo
    {
        public static Dictionary<int, UIInfo> UIInfoDic
        {
            get;
        } = new Dictionary<int, UIInfo>();
        ...
        public bool CanAndroidBack = true;

        public UIInfo(string assetName, Type viewClass, UIViewInfo info,
            EnumMaskStatus maskStatus = EnumMaskStatus.kNone, UIInfo[] uiInfos = null, int uiId = 0,int group=0,bool isCheckPop = true,bool canAndroidBack= true)
        {
            ...
            this.group = group;
            this.isJumpPopCheck = isCheckPop;
            this.CanAndroidBack = canAndroidBack;
