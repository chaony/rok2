...
namespace Skyunion
{
    internal class InputManager : Module, IInputManager
    {
        ...
        private GameObject m_touch3DCollider = null;
        ...
        private Action<int, int> m_eventTouchBeginAction;
        private Action<int, int> m_eventTouchMoveAction;
        private Action<int, int> m_eventTouchEndAction;
            ...
            if (flag)
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    if (!isTouching && num >= 0 && num <= Screen.width && num2 >= 0 && num2 <= Screen.height)
                    {
                        ...
                        m_eventTouchBeginAction?.Invoke(num, num2);
                    }
                }
                else if (Input.GetMouseButton(0))
                {
                    ...
                    if (num3 >= 0 && num3 <= Screen.width && num4 >= 0 && num4 <= Screen.height && (isMoving || Mathf.Abs(lastTouchX - num3) > 10 || Mathf.Abs(lastTouchY - num4) > 10) && (num3 != lastTouchX || num4 != lastTouchY))
                    {
                        ...
                        m_eventTouchMoveAction?.Invoke(num3, num4);
                    }
                }
                else if (Input.GetMouseButtonUp(0))
                {
                    ...
                    if (isTouching)
                    {
                        ...
                        m_eventTouchEndAction?.Invoke(x, y);
                    }
}
                else
                {
        ...
        public bool TouchBegan3DCheck(int x, int y)
        {
            ...
            //    m_touch3DCollider = raycastHit.transform.gameObject;
            ...
            //}
            int num = -1;
            RaycastHit[] array = RayCashHit3D(x, y, out num);
            ...
            if (num > -1)
            {
                ...
                if (raycastHit.transform.parent != null)
                {
                    ...
                    m_OnTouchBegan3DCheckAction?.Invoke(x, y, m_touch3DGameobjectId, collider.name);
                    return true;
                }
            }
            m_touch3DGameobjectId = string.Empty;
            m_touch3DCollider = null;
            return false;
        }
        ...
        public bool TouchEnded3DCheck(int x, int y)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                if (m_touch3DCollider == array[i].transform.gameObject)
                {
                    flag = true;
                    var collider = array[i].collider;
                    colliderId = collider.name;
                    m_On3DCheckAction?.Invoke(x, y, text, colliderId);
                    break;
                }
            }
            if (!flag && m_touch3DCollider != null)
            {
        ...
        public void AddTouchEvent(Action<int, int> eventOnToucheBegin, Action<int, int> eventOnToucheMove, Action<int, int> eventOnToucheEnd)
        {
            m_eventTouchBeginAction += eventOnToucheBegin;
            m_eventTouchMoveAction += eventOnToucheMove;
            m_eventTouchEndAction += eventOnToucheEnd;
        }

        public void RemoveTouchEvent(Action<int, int> eventOnToucheBegin, Action<int, int> eventOnToucheMove, Action<int, int> eventOnToucheEnd)
        {
            m_eventTouchBeginAction -= eventOnToucheBegin;
            m_eventTouchMoveAction -= eventOnToucheMove;
            m_eventTouchEndAction -= eventOnToucheEnd;
        ...
        public RaycastHit[] RayCashHit3D(int x, int y, out int nIndex)
        { 
            Ray ray = Camera.main.ScreenPointToRay(new Vector3((float)x, (float)y, 0f));
            int num = -1;
            int num2 = LayerMask.NameToLayer("RaycastCollider");
