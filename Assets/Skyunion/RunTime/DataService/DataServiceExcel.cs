...
namespace Skyunion
{
    ...
    class DataServiceExcel : DataService
    {
        ...
        bool LoadFile()
        {
            ...
            var files = (Directory.GetFiles(m_ExcelDir, "*.*", SearchOption.TopDirectoryOnly)
                .Where(s => withoutExtensions.Contains(Path.GetExtension(s).ToLower())).ToList());

            files.Sort();

            mExcelChangeCount = 0;
            TableInfo globalTable = null;
            foreach (var path in files)
            {
                ...
                if (m_mapTables.TryGetValue(fileName, out table))
                {
                    ...
                    if (modifyTime == table.modifyTime && fileSize == table.fileSize)
                    {
                        ...
                        if(globalTable != null)
                        {
                            ...
                            {
                                if(globalTable.objectType.ContainsKey(field.basetype))
                                {
                ...
                if (table != null)
                {
                    if (table.isHasChange == false && md5 == table.md5)
                    {
            ...
            CoreUtils.logService.Info($"End Analysis Excel:{mExcelChangeCount} Excel Changed! {Time.realtimeSinceStartup - fStart}", Color.green);
            return true;
        }
        ...
        bool ReplaceTextColor()
        {
            ...
            writer.Write((ushort)table.field.Count);
        ...
        bool LoadCache()
        {
            ...
            {
                version = Convert.ToInt32(rootNode.GetAttribute("Version"));
            }
            if(version != VERSION)
            {
                CoreUtils.logService.Info($"Excel Version Change ! {version}->{VERSION}", Color.green);
                return true;
            }
            foreach (var excelNode in rootNode.ChildNodes)
            {
                ...
                foreach(var childNode in excelElement.ChildNodes)
                {
                    ...
                    if (childElement.Name == "Field")
                    {
                        ...
                        field.list = IsArrayType(field.type);
        ...
        string GetBaseType(string type)
        {
            if (type[0] == 'L')
            {
                int nIdx = type.IndexOf("List<");
                if (nIdx != -1)
                {
                    int nIdx2 = type.LastIndexOf(">");
                    return type.Substring(nIdx + 5, nIdx2 - 5 - nIdx);
                }
            }
            return type;
        }
            ...
            var ds = excelReader.AsDataSet(new ExcelDataSetConfiguration()
            {
                ...
                ConfigureDataTable = (reader)=>
                {
                    ...
                    if (reader.Name[0] != '@')
                    {
                        ...
                        {
                            FilterColumn = (col, idx) =>
                            {
                                if(colCount == -1)
                                {
                                    colCount = col.FieldCount;
                                    for (int i = 0; i < col.FieldCount; i++)
                                    {
                                        string name = col.GetString(i);
            ...
            for (int i = 0; i < ds.Tables.Count; i++)
            {
                ...
                if (table.isVertical)
                {
                    for (int row = 1; row < sheet.Rows.Count; row++)
                    {
                        ...
                        fieldInfo.list = IsArrayType(fieldInfo.type);
