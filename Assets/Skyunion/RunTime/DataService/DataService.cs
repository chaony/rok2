...
namespace Skyunion
{
    ...
    public abstract class TableBase<T> : ITable<T>
    {
        ...
        protected T [] records;
        protected List<T> listRecords = null;
        ...
        public T QueryRecord(int id)
        {
            ...
            if (mapIdx.TryGetValue(id, out idx))
            {
                ...
                if (value == null)
                {
                    value = records[idx] = ConvertToObject(rows[idx]);
                }
            }
            else
            {
        ...
        public List<T> QueryRecords()
        {
            if (fullRecords == false)
            {
                ...
                for (int i = 0; i < records.Length; i++)
                {
                    if (records[i] != null)
                        continue;
                    records[i] = ConvertToObject(rows[i]);
                }
                listRecords = new List<T>(records);
            }
            return listRecords;
        }
        public string GetString(Type type, object value)
        {
            ...
            else if (typeof(IList).IsAssignableFrom(type))
            {
                ...
                {
                    content += GetString(elem_type, list[i]); ;
                    if (i != list.Count - 1)
                    {
                        content += "|";
                    }
        ...
        public object ReadValue(Type type, string content)
        {
            object value;
            if (typeof(string) == type)
            {
            ...
            else if (typeof(float) == type)
            {
                ...
                {
                    value = Convert.ToSingle(content, System.Globalization.CultureInfo.InvariantCulture);
                }
            }
            else if (typeof(TimeSpan) == type)
            {
        ...
        public T ConvertToObject(string [] cols)
        {
            var col = (T)Instantiate(type);

            if (cols.Length != fields.Length + props.Length)
            {
                Debug.LogError(type.FullName + " dataCols: "+ cols.Length + " != cs.fields:"+(fields.Length + props.Length)+ "id:" + cols[0]);
            }
