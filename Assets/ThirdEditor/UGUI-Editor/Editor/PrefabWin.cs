...
public class PrefabWin : EditorWindow
{
	...
	int GetCellUnderMouse (int spacingX, int spacingY)
	{
		...
		float width = position.width- cellPadding + mPos.x;
		float height = position.height - cellPadding + mPos.y;
		int index = 0;
	...
	void OnGUI ()
	{
		...
		int width = (int)this.position.width;
		...
		int indexUnderMouse = mCellIndex;

		if (mMouseIsInside == false && mCellIndex == -1)
		{
			indexUnderMouse = GetCellUnderMouse(spacingX, spacingY);
		}
		...
		bool eligibleToDrag = true;// (currentEvent.mousePosition.y < position.height - 40);

		if (type == EventType.MouseDown)
		{
			mMouseIsInside = true;
			mCellIndex = indexUnderMouse;
		}
        ...
        mPos = EditorGUILayout.BeginScrollView(mPos);
		{
			...
			for (int i = 0; i < indices.size; ++i)
			{
				...
				string caption = (ent == null) ? "" : ent.prefab.name.Replace(Configure.TemplateNameExt, "");
				...
				if (ent != null)
				{
					...
					if (ent.tex != null)
					{
						...
						labelPos.y = inner.y + inner.height - labelPos.height + cellPadding;
						labelStyle.fontSize = (int) (_labelDefaultFontSize * SizePercent);
						labelStyle.alignment = TextAnchor.LowerCenter;
						{
							GUI.Label(labelPos, ent.prefab.name.Substring(Configure.TemplateNameExt.Length+1),labelStyle);
						}
...
//						labelStyle.fontSize = _labelDefaultFontSize;
        ...
        //else if (mTab != 0)
        {
            GUILayout.BeginHorizontal();
            {
                string after = EditorGUILayout.TextField("", searchFilter, "SearchTextField", GUILayout.Width(position.width - 20f));
