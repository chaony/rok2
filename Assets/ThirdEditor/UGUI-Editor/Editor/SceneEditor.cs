                    ...
                    if (selectPos == null || !selectPos.Equals(e.mousePosition))
                    {
                        ...
                        foreach (var obj in _selecedObjs)
                        {
                            ...
                            if (trt != null)
                            {
                                ...
                                {
                                    if (trt.parent && (trt.parent.name.Equals("Container") || trt.parent.name.Equals("TipLayer")))
                                    {
                                    }
                                    else
                                    {
...
//                        Selection.activeTransform.SetParent(UIEditorHelper.GetGoodContainer(last_trans), false);
        ...
        else if (Event.current != null && Event.current.button == 1 && Event.current.type == EventType.MouseUp && Configure.IsShowSceneMenu && sceneView.in2DMode)
        {
