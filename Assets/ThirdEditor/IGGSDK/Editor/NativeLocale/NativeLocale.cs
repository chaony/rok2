...
{
    public static void AddLocalizedStringsIOS(string projectPath, string localizedDirectoryPath, string defaultLocal = "en")
    {        
        ...
        {
            var loccalName = Path.GetFileNameWithoutExtension(sub.Name);
            if (loccalName.Equals(defaultLocal))
            {
                locales.Insert(0, loccalName);
            }
            else
            {
                locales.Add(loccalName);
            }
        }
    ...
    public static void AddLocalizedStringsIOS(string projectPath, string localizedDirectoryPath, List<string> validLocales)
    {
        ...
        proj.ClearLocalization("InfoPlist.strings");
